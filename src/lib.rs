    ...
    fn send(&self, command: &[u8]) -> Result<Vec<u8>, OramError> {
        match bincode::deserialize::<Command>(command).unwrap() {
            Command::Server1Write(ct, f, k_oram_t, cs) => {
                self.s1.write().unwrap().write(ct, f, k_oram_t, cs)?;
                Ok(vec![])
            }
            Command::Server2Write(write_type) => {
                match write_type {
                    WriteType::Write(buckets) => {
                        self.s2.write().unwrap().write(buckets);
                        Ok(vec![])
                    }
                    WriteType::AddPrfKey(key) => {
                        self.s2.write().unwrap().add_prf_key(&key);
                        Ok(vec![])
                    }
                    WriteType::SavePathset(pathset) => {
                        self.s2.write().unwrap().save_pathset(pathset);
                        Ok(vec![])
                    }
                }
            }
...
impl Client {
    pub fn new(id: String, s1: Arc<RwLock<Server1>>, s2: Arc<RwLock<Server2>>) -> Self {
        Client {
            id,
            epoch: 0,
            keys: HashMap::new(),
...
mod e2e_tests {
    ...
    fn test_message_persistence() {
        let server2 = Arc::new(RwLock::new(Server2::new()));
        let server1 = Arc::new(RwLock::new(Server1::new(server2.clone())));
        ...
        for epoch in 0..num_epochs {
            server1.write().unwrap().batch_init(num_clients);
            let message: Vec<u8> = (0..16).map(|_| (rng.next_u32() % 255 + 1) as u8).collect();
            client.write(&message, &key).unwrap();
            server1.write().unwrap().batch_write().unwrap();
            messages.push(message);
        }

        // Verify the messages
        let mut decrypted_messages = Vec::new();
        let _ = server2.read().unwrap().tree
            .zip(&server1.read().unwrap().metadata)
            .into_iter()
    ...
    fn test_message_movement() {
        ...
        // Initial write
        server1.write().unwrap().batch_init(1);
        ...
        client.epoch += 1;
        client.s1.write().unwrap().write(ct, f.clone(), Key::new(k_oram_t), cs.clone()).expect("Initial write failed");
        let k_s1_t = server1.write().unwrap().k_s1_t.0.clone();
        let l = prf(k_s1_t.as_slice(), &[f.clone().as_slice(), cs.clone().as_slice()].concat());
        let intended_path = Path::from(l);

        server1.write().unwrap().batch_write().expect("Initial batch write failed");

        let mut pathset: tree::SparseBinaryTree<Bucket> = server1.read().unwrap().pt.clone();

        // Function to verify message at LCA
        let verify_message_at_lca = |lca_bucket: &Bucket, lca_path: &Path| {
            let metadata_bucket = server1.read().unwrap().metadata.get(lca_path)
        ...
        let mut latest_index = server2.read().unwrap().tree.get_index(&lca_path);
        ...
        for epoch in 1..num_epochs {
            ...
            // Perform batch_write
            server1.write().unwrap().batch_write().expect("Batch write failed");

            let mut new_pathset: tree::SparseBinaryTree<Bucket> = server1.read().unwrap().pt.clone();
    ...
    fn test_tree_serialization() {
        ...
        let server2_tree_serialized = bincode::serialize(&server2.read().unwrap().tree).unwrap();
        ...
        assert_eq!(server2.read().unwrap().tree, server2_tree_deserialized);
    }

    /// Helper function to test the execution of the protocol over a user-defined number of clients and epochs.
    fn test_protocol_execution_with_params(s1: Arc<RwLock<Server1>>, s2: Arc<RwLock<Server2>>, num_clients: usize, num_epochs: usize){
        let mut rng = ChaCha20Rng::from_entropy();
